...
def load_udiff_from_local_zip(path="Databases/UDiFF.zip"):
    try:
        ...
        print("❌ Failed to extract or read UDiFF:", e)
        return None
...
if __name__ == "__main__":
    ...
    trade_date = get_last_trading_day()
    bhav_df = load_udiff_from_local_zip()
    if bhav_df is not None:
        live_df = generate_pcr_data(bhav_df, symbols, trade_date)
        if not live_df.empty:
            update_excel(historical_file, live_df)
        ...
        print("⚠️ Failed to read UDiFF from local ZIP.")
