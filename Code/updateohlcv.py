...
if not os.path.exists(excel_path):
    logging.error(f"{excel_path} does not exist!")
...
for sheet in sheet_names:
    ...
    try:
        ...
        if existing_df.empty or 'DATE' not in existing_df.columns:
            logging.warning(f"No data or 'DATE' column missing in {sheet}. Skipping.")
            continue
        ...
        if fetch_from > today:
            ...
            continue
        logging.info(f"Fetching new data from {fetch_from} to {today}")
        ...
        logging.info(f"Updated rows: {len(yf_data)}")
...
logging.info(f"All sheets updated in: {excel_path}")
